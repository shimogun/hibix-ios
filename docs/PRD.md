...
CREATE TABLE emergency_contacts (
    id INTEGER PRIMARY KEY AUTOINCREMENT,
    email TEXT NOT NULL,                      -- 連絡先の値 (種別が email 以外でも本カラムに格納・v0.1 後方互換)
    label TEXT,                               -- 表示名（例: 'お母さん'）NULL可
    sort_order INTEGER NOT NULL DEFAULT 0,
    created_at TEXT NOT NULL,
    contact_type TEXT NOT NULL DEFAULT 'email' -- v2 migration: 'email' | 'line' | 'phone'
);
```
...
**Entitlement**: 全層
**UI 要件**:
- 気分タップ後にシート（`.sheet`）で表示。メモ入力をスキップして閉じることも可
- `TextEditor` を中央配置、フォーカス自動取得
- 右上に「保存」、左上に「戻る」（戻るで閉じた場合は memo 未変更）
...
**緊急連絡先**:
- 最大3件まで登録可
- 連絡種別（`email` / `line` / `phone`）+ 連絡先の値（必須）+ 表示ラベル（任意・例「お母さん」）
- 端末側ローカル DB に平文保存、サーバー側は AES-256-GCM 暗号化保存
- **v0.1 では `email` のみ実送信**。`line` / `phone` は登録のみ受け付け、実送信は v0.2 で対応予定（UI 上で注記表示）
- 種別ごとのバリデーション:
  - `email`: 簡易 RFC 準拠（`@` 1個 + ドメイン部に英字2文字以上の TLD）
  - `line`: 空でなければ可（LINE ID または URL を許容）
  - `phone`: 数字・ハイフン・スペース・括弧・`+` のみ、数字 5 桁以上
