...
**緊急連絡先**:
- 最大3件まで登録可
- 連絡種別（`email` / `line`）+ 連絡先の値（必須）+ 表示ラベル（任意・例「お母さん」）
- 端末側ローカル DB に平文保存、サーバー側は AES-256-GCM 暗号化保存
- **v1.0 では `email` のみ実送信**。`phone`（電話）は廃止（v1.0 で UI・enum・バリデーションから完全撤去）。`line` は登録のみ受け付け、実送信は v1.1（Messaging API・公式アカウント方式）で対応予定（UI 上で注記表示）
- 種別ごとのバリデーション:
